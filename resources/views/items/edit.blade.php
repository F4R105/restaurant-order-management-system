<x-auth-layout title="Edit item">
    <div class="mb-8">
        <a href="{{ route('items.show', $item) }}" class="inline-flex items-center text-sm font-medium text-zinc-500 hover:text-zinc-800 transition-colors mb-3">
            <svg class="w-4.5 h-4.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Back to item details
        </a>
        <h1 class="text-3xl font-bold text-zinc-950 tracking-tight">Edit Item</h1>
    </div>
        ...
        <div class="p-8">
            <form action="{{ route('items.update', $item) }}" method="post" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" name="unit_price" id="unit_price" placeholder="Unit price" value="{{ $item->unit_price }}" required
                        class="block w-full px-3.5 py-2.5 bg-white border border-zinc-300 rounded-xl shadow-xs placeholder-zinc-400 focus:outline-hidden focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm transition-all duration-200">
                    @error('unit_price')
